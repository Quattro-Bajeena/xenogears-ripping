...
void parse_file(unsigned long start_sector, unsigned long file_size, int id, char *file_name)
{
    ...
    file_position = fseek(infile, 2048L * start_sector, 0);
    // skip sector header
...
void create_index_file()
{
    ...
    file_position = fseek(infile, 24L * 2048L, 0);
...
void extract(char *filename)
{
    ...
    for (i = 0; ; i++)
    {
        ...
        // directory
        if ((file_size & 0x80000000) != 0)
        {
            // create directory name (name - number of directory)
	        number_in_directory = 0;
            filename[9] = 0;
            ...
            printf("number of file %d, in dir %d\n", static_cast<int>(-file_size), directory_number);
        } else if (start_sector != 0 && file_size > 0) {
            printf("file number %d, size: 0x%x byte - ", number_in_directory, static_cast<unsigned int>(file_size));
            parse_file(start_sector, file_size, number_in_directory, filename);
	    number_in_directory += 1;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        printf("USAGE:'extract.exe isofile disc' extract files from xeno cd image.\n");
        return EXIT_FAILURE;
    }
